...
uses
  Classes, SysUtils, ssockets, memcard;
...
type
  ...
  public
    ...
    procedure SysStat;
    procedure CopyFrom(src: TMemCard; blkid: integer);
...
begin
  ...
  if buf.op = 120 then
    FSock.Read(buf, SizeOf(buf));
  if buf.op <> 200 then
    Raise EAuthError.Create('Access Denied: '+IntToStr(buf.op));
...
begin
  ...
  FSock.Read(buf, SizeOf(buf));
  if buf.op <> 90 then
    Raise EAuthError.Create('Bad Op: '+IntToStr(buf.op));
  blkid:=@buf.data[0];
  Result:=blkid^;
  if blkid^ > 0 then
...
procedure TNetCard.CopyFrom(src: TMemCard; blkid: integer);
var
  blk: TMemoryStream;
  info: PBlockInfo;
  nb: integer;
begin
  ...
  if nb < 1 then
    Exit;
  New(info);
  src.GetInfo(blkid, info);
  blk:=src.ReadBlock(blkid);
  WriteBlock(nb, blk, info);
  blk.Free;
  Dispose(info);
end;

procedure TNetCard.CopyBlock(blkid: integer; target: TMemCard);
var
  blk: TMemoryStream;
  info: PBlockInfo;
  nb: integer;
...
procedure TNetCard.CopyCard(target: TMemCard);
var
  i: integer;
begin
  for i:=1 to 15 do
